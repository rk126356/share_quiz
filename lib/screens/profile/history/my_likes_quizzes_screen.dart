...
class MyLikedQuizzesScreen extends StatefulWidget {
  const MyLikedQuizzesScreen({super.key});

  @override
  State<MyLikedQuizzesScreen> createState() => _MyLikedQuizzesScreenState();
}

class _MyLikedQuizzesScreenState extends State<MyLikedQuizzesScreen> {
  ...
  Future<void> fetchQuizzes(bool next, context) async {
    ...
    if (quizItems.isNotEmpty) {
      ...
      likedQuizzesCollection = await firestore
          .collection('users/${data.userData.uid}/myLikedQuizzes')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: const Text('My Liked Quizzes'),
        ),
        body: _isLoading
            ? const LoadingWidget()
            : quizItems.isEmpty
                ? const Center(
                    child: Text('You have not liked any quiz yet.'),
                  )
                : SizedBox(
