...
class MySharedQuizzesScreen extends StatefulWidget {
  const MySharedQuizzesScreen({super.key});

  @override
  State<MySharedQuizzesScreen> createState() => _MySharedQuizzesScreenState();
}

class _MySharedQuizzesScreenState extends State<MySharedQuizzesScreen> {
  ...
  Future<void> fetchQuizzes(bool next, context) async {
    ...
    if (quizItems.isNotEmpty) {
      ...
      likedQuizzesCollection = await firestore
          .collection('users/${data.userData.uid}/mySharedQuizzes')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: const Text('My Shared Quizzes'),
        ),
        body: _isLoading
            ? const LoadingWidget()
            : quizItems.isEmpty
                ? const Center(
                    child: Text('You have not shared any quiz yet.'),
                  )
                : SizedBox(
