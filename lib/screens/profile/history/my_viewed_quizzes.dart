...
class MyViewedQuizzesScreen extends StatefulWidget {
  const MyViewedQuizzesScreen({super.key});

  @override
  State<MyViewedQuizzesScreen> createState() => _MyViewedQuizzesScreenState();
}

class _MyViewedQuizzesScreenState extends State<MyViewedQuizzesScreen> {
  ...
  Future<void> fetchQuizzes(bool next, context) async {
    ...
    if (quizItems.isNotEmpty) {
      ...
      likedQuizzesCollection = await firestore
          .collection('users/${data.userData.uid}/myViewedQuizzes')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: const Text('My Viewed Quizzes'),
        ),
        body: _isLoading
            ? const LoadingWidget()
            : quizItems.isEmpty
                ? const Center(
                    child: Text('You have not viewed any quiz yet.'),
                  )
                : SizedBox(
