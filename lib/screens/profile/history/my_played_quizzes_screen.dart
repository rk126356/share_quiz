...
class MyPlayedQuizzesScreen extends StatefulWidget {
  const MyPlayedQuizzesScreen({super.key});

  @override
  State<MyPlayedQuizzesScreen> createState() => _MyPlayedQuizzesScreenState();
}

class _MyPlayedQuizzesScreenState extends State<MyPlayedQuizzesScreen> {
  ...
  Future<void> fetchQuizzes(bool next, context) async {
    ...
    if (quizItems.isNotEmpty) {
      ...
      likedQuizzesCollection = await firestore
          .collection('users/${data.userData.uid}/myPlayedQuizzes')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: const Text('My Played Quizzes'),
        ),
        body: _isLoading
            ? const LoadingWidget()
            : quizItems.isEmpty
                ? const Center(
                    child: Text('You have not played any quiz yet.'),
                  )
                : SizedBox(
